...
use crate::prelude::{ByteIdx, GraphemeType, ColIdx, GraphemeIdx, WordSeparatorType, WordType};
...
impl Line {
    ...
    pub fn current_or_next_word_end_grapheme_idx(&self, current_grapheme_idx: GraphemeIdx, separator_type: WordSeparatorType, start_at_prev: bool, reverse: bool) -> Option<GraphemeIdx> {
        ...
        let mut f = |i: usize, s: &str, reverse: bool| {
            ...
            match grapheme_type {
                ...
                GraphemeType::Whitespace => {
                    if word_type != WordType::None {
                        if (grapheme_record.0 + grapheme_record.1) > 1 || (grapheme_record.2 && (grapheme_record.0 + grapheme_record.1 ) == 1) {
                            return Some(target_idx);
                        }
                    }
                    grapheme_record.2 = true;
        ...
        if reverse {
            for (i, t) in enumerate.rev().skip(self.fragments.len().saturating_sub(1).saturating_sub(current_grapheme_idx)) {
    ...
    pub fn next_word_start_grapheme_idx(&self, current_grapheme_idx: GraphemeIdx, separator_type: WordSeparatorType, start_at_prev: bool) -> Option<GraphemeIdx> {
        if self.fragments.len() == 0 {
            return None;
        }
        ...
        for (i, t) in self.fragments.iter().enumerate().skip(current_grapheme_idx) {
            ...
            match grapheme_type {
                GraphemeType::Keyword => {
                    if grapheme_record.2 || (matches!(separator_type, WordSeparatorType::Symbol) && grapheme_record.1) {
                        return Some(i);
                    }
    ...
    pub fn get_annotated_visible_substr(&self, range: Range<ColIdx>, annotations: Option<&Vec<Annotation>>) -> AnnotatedString {
        ...
        for fragment in self.fragments.iter().rev() {
            ...
            if fragment_end <= range.start {
                ...
                result.replace(0, fragment.start.saturating_add(fragment.grapheme.len()), "...");
                break;
            }
    ...
    pub fn delete_word_forward_from(&mut self, at: GraphemeIdx) -> GraphemeIdx {
        ...
        let end = self.current_or_next_word_end_grapheme_idx(at, WordSeparatorType::Symbol, false, false)
    ...
    pub fn delete_word_backward_from(&mut self, at: GraphemeIdx) -> GraphemeIdx {
        ...
        let start_idx = at.saturating_sub(1).min(self.grapheme_count().saturating_sub(1));
        let start = self.current_or_next_word_end_grapheme_idx(start_idx, WordSeparatorType::Symbol, true, true)
            .unwrap_or(0);
    ...
    pub fn search_backward(&self, query: &str, from_grapheme_idx: GraphemeIdx) -> Option<GraphemeIdx> {
        ...
        self.find_all(query, 0..end)
            .first().map(|(_, grapheme_idx)| *grapheme_idx)
    }
    ...
    pub fn find_all(&self, query: &str, range: Range<ByteIdx>) -> Vec<(ByteIdx, GraphemeIdx)> {
        ...
        self.string.get(start..end)
            .map_or_else(Vec::new, |substr| {
                let potential_matches : Vec<ByteIdx> = substr.match_indices(query)
            ...
            .filter_map(|&start| {
                self.byte_idx_to_grapheme_idx(start)
                    .and_then(|grapheme_idx| {
                        self.fragments.get(grapheme_idx..grapheme_idx.saturating_add(grapheme_count))
                            .and_then(|fragments| {
                                let substring = fragments.iter()
