...
use editor_core::prelude::{ColIdx, Location, Position, RowIdx, Size, WordSeparatorType};
use editor_core::renderer::Renderer;
use editor_core::search_direction::SearchDirection;
use editor_core::search_info::SearchInfo;
use std::cmp::min;
use std::io::Error;
...
impl Default for View {
    fn default() -> Self {
        Self {
            ...
            last_search_direction: SearchDirection::default(),
            last_find_char: None,
            last_find_kind: FindKind::default(),
            last_edit: None,
...
impl View {
    /// 创建默认 View，并设置行号显示为开启。
    pub fn default_with_line_numbers() -> Self {
        Self::default()
    }
    ...
    pub fn handle_move_command(&mut self, command: Move) {
        ...
        match command {
            ...
            Move::NextWordSplitByWhitespace => {
                self.move_to_next_word_start(WordSeparatorType::Whitespace)
            }
            Move::EndOfWordSplitBySymbol => {
                self.move_to_current_or_next_word_end(WordSeparatorType::Symbol)
            }
            Move::EndOfWordSplitByWhitespace => {
                self.move_to_current_or_next_word_end(WordSeparatorType::Whitespace)
            }
            Move::PrevWordSplitBySymbol => {
                self.move_to_current_or_prev_word_start(WordSeparatorType::Symbol)
            }
            Move::PrevWordSplitByWhitespace => {
                self.move_to_current_or_prev_word_start(WordSeparatorType::Whitespace)
            }
    ...
    fn move_to_next_word_start(&mut self, separator_type: WordSeparatorType) {
        self.text_location = self
            .buffer
            .next_word_location_start(&self.text_location, separator_type);
    }

    fn move_to_current_or_next_word_end(&mut self, separator_type: WordSeparatorType) {
        self.text_location = self
            .buffer
            .current_or_next_word_location_end(&self.text_location, separator_type);
    }

    fn move_to_current_or_prev_word_start(&mut self, separator_type: WordSeparatorType) {
        self.text_location = self
            .buffer
            .current_or_prev_word_location_start(&self.text_location, separator_type);
    }
    ...
    fn move_find_char(&mut self, ch: char, kind: FindKind) {
        ...
        let result = match kind {
            ...
            FindKind::TillForward => self
                .buffer
                .find_char_on_line(line_idx, from, ch, true)
                .and_then(|idx| {
                    if idx > 0 {
            ...
            FindKind::TillBackward => self
                .buffer
                .find_char_on_line(line_idx, from, ch, false)
                .and_then(|idx| {
    ...
    fn toggle_case(&mut self) {
        ...
        if let Some(line) = self.buffer.get_line_content(saved.line_idx) {
            let byte_idx = self
                .buffer
                .grapheme_to_byte(saved.line_idx, saved.grapheme_idx);
    ...
    fn delete_selection(&mut self) {
        ...
        if let Some((start, end)) = self.selection_range() {
            if self.selection_linewise {
                ...
                self.text_location = Location {
                    line_idx: start.line_idx.min(self.buffer.last_line_idx()),
                    grapheme_idx: 0,
                };
            } else {
                ...
                if start.line_idx == end.line_idx {
                    ...
                    if let Some(line) = self.buffer.get_line_content(start.line_idx) {
                        let sb = self
                            .buffer
                            .grapheme_to_byte(start.line_idx, start.grapheme_idx);
                        let eb = self
                            .buffer
                            .grapheme_to_byte(start.line_idx, end.grapheme_idx);
    ...
    fn move_to_matching_bracket(&mut self) {
        ...
        if let Some(line) = self.buffer.get_line_content(self.text_location.line_idx) {
            let byte_idx = self
                .buffer
                .grapheme_to_byte(self.text_location.line_idx, self.text_location.grapheme_idx);
            if let Some(&ch) = line.as_bytes().get(byte_idx) {
                let ch = ch as char;
                ...
                self.buffer
                    .find_matching_bracket(self.text_location, open, close, forward)
                    .map(|loc| {
    ...
    pub fn get_word_under_cursor(&mut self) -> Option<String> {
        ...
        let start = self
            .buffer
            .current_or_prev_word_location_start(&saved, WordSeparatorType::Symbol);
        // Then find the end
        let end = self
            .buffer
            .current_or_next_word_location_end(&saved, WordSeparatorType::Symbol);
        ...
        if let Some((start, end)) = self.buffer.inner_word_range(&self.text_location) {
            ...
            let last = if end.grapheme_idx > 0 {
                Location {
                    ...
                    grapheme_idx: end.grapheme_idx.saturating_sub(1),
                }
            } else {
                start
            };
    ...
    fn selection_range(&self) -> Option<(Location, Location)> {
        ...
        if self.selection_linewise {
            ...
            Some((
                ...
                Location {
                    line_idx: c.line_idx,
                    grapheme_idx: self.buffer.grapheme_count(c.line_idx),
                },
            ))
    ...
    fn command_load(&mut self, filename: &str) -> String {
        ...
        if result.is_ok() {
            ...
            format!(
                "Fail to open file: {filename:?}, cause by: {}",
        ...
        match command_value {
            CommandValue::Quit => {
                if self.get_status().is_modified {
                    Some(
                        "Change unsaved, Please save before quit or use \"q!\" force quit"
                            .to_string(),
                    )
                } else {
                    None
            ...
            CommandValue::SaveAs(filename) => {
                ...
                if result.is_ok() {
                    ...
                    Some(format!(
                        "Fail to save as file: {filename:?}, cause by: {}",
                        result.unwrap_err().to_string()
                    ))
                }
            }
            ...
            CommandValue::ForceOpen(filename) => Some(self.command_load(filename.as_str())),
    ...
    fn get_search_query(&self) -> Option<&Line> {
        ...
        debug_assert!(
            query.is_some(),
            "Attempting to search without malformed search info present"
        );
        query
    }
    ...
    fn delete_to_next_word_start(&mut self) {
        ...
        let count = if let Some(target) = self
            .buffer
            .find_next_word_start_on_line(line_idx, self.text_location.grapheme_idx)
        {
    ...
    fn is_same_line_motion(cmd: Move) -> bool {
        matches!(
            ...
            Move::Left
                ...
                | Move::FindChar(_)
                | Move::FindCharBackward(_)
                | Move::TillChar(_)
                | Move::TillCharBackward(_)
                | Move::RepeatFindSameDir
                | Move::RepeatFindOppositeDir
        )
    }
    ...
    fn delete_range_to_motion(&mut self, move_cmd: Move) {
        ...
        let mut end = if same_line {
            self.cap_same_line_end(start, self.text_location)
        } else {
            self.text_location
        };
        ...
        if matches!(
            ...
            let gc = self.buffer.grapheme_count(end.line_idx);
            if end.grapheme_idx < gc {
                end.grapheme_idx += 1;
            }
        }
    ...
    fn delete_range_inner(&mut self, start: Location, end: Location) {
        if start.line_idx == end.line_idx {
            ...
            for _ in start.line_idx.saturating_add(1)..end.line_idx {
                self.text_location = Location {
                    line_idx: start.line_idx.saturating_add(1),
                    grapheme_idx: 0,
                };
                self.delete_line();
            }
...
impl UIComponent for View {
    ...
    fn draw(&mut self, renderer: &mut dyn Renderer, origin_row: RowIdx) -> Result<(), String> {
        ...
        renderer.set_cursor_line(self.text_location.line_idx);
        renderer.set_show_line_numbers(self.show_line_numbers);
        let query = self
            .search_info
            .as_ref()
        ...
        let buf_height = self.buffer.height();
        let highlight_end = (end_y.saturating_add(scroll_top)).min(buf_height);
        for current_row in 0..highlight_end {
        ...
        for current_row in origin_row..end_y {
            ...
            if let Some(mut annotated_string) =
                self.buffer
                    .get_highlight_substring(line_idx, left..right, &highlighter)
            {
                if let Some((sel_start, sel_end)) = sel_range {
                    if line_idx >= sel_start.line_idx && line_idx <= sel_end.line_idx {
                        let scroll_byte = self.buffer.column_to_byte(line_idx, left);
                        let start_g = if line_idx == sel_start.line_idx {
                            sel_start.grapheme_idx
                        } else {
                            0
                        };
                ...
                renderer.render_annotated_row(current_row, line_idx, &annotated_string)?
            } else if line_idx == 0 {
                // 第一行始终显示行号，无论是否有内容
                let empty = AnnotatedString::from("");
                renderer.render_annotated_row(current_row, line_idx, &empty)?
            } else {
                // 超出 buffer 的空行不显示行号
                renderer.render_text_row(current_row, "")?
            }
        }
        Ok(())
    }
